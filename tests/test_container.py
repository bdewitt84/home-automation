...
def test_validate_graph_dfs_no_cycle(container):
    ...
    # DFS validation only reads metadata, no __init__ needed
    ...
    data = [
        (DependencyE, {}),
        (DependencyD, {}),
        (DependencyC, {"dep_e": DependencyE}),
        (DependencyB, {"dep_d": DependencyD, "dep_e": DependencyE}),
        (DependencyA, {"dep_b": DependencyB, "dep_c": DependencyC}),
    ]

    for cls, req in data:
        meta = generate_metadata(cls=cls, requirements=req)
        container.register_component(key=cls.__name__, cls=cls, metadata=meta, overrides={})
...
def test_validate_graph_dfs_with_cycle(container):
    ...
    data = [
        (DependencyC, {"dep_a": DependencyA}),
        (DependencyB, {"dep_c": DependencyC}),
        (DependencyA, {"dep_b": DependencyB}),
    ]
...
def test_validate_graph_dfs_dependency_not_found(container):
    ...
    meta_b = generate_metadata(cls=DependencyB, requirements={"dep_a": DependencyA})
    container.register_component(key=DependencyB.__name__, cls=DependencyB, metadata=meta_b, overrides={})
